...
export async function GET(request: Request) {
	...
	const feed = new Feed({
		...
		description:
			'Life in One Room is part of the project CRIP, produced by Konstfrämjandet Västmanland, with generous funding by Kulturbryggan',
		id: 'lifeinoneroom',
	...
	allPosts.forEach((post) => {
		...
		const description =
			(post.__typename === 'ImageRecord' ? post.textToVoice : render(post.text as any)).slice(0, 200) + '...';
